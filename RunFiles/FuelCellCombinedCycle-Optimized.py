...
from ParallelCycles import OptimizeCombinedCycle,ReplicateCombinedCycleInputs,WriteAdditionalObjects,WriteResultObject
...
###define the parameters to be optimized, fixed###
...
NumberOfEngines=6		#includes the topping cycle. note, this is copied to another variable below in this particular script.
#set how to optimize the cycle
CombinedOptimization=True

#assign some line colors for each engine in the cascade
LineColors=['black','red','green','blue','purple','brown','yello']
...
#limit the number of engines to allow in the combined cycle
NonCO2CycleFixedIndependentVariableValues+=(NumberOfEngines,)		#includes the topping cycle
...
NonCO2CycleFixedIndependentVariableValues+=[37.15,]
...
NonCO2CycleFixedIndependentVariableValues+=[0.80,]#note, fuel utilizations lower than 0.6 result in temperatures above 1724.0, which REFPROP functions can't seem to handle, although the GUI seems to okay!!!!!! if set water fraction to zero and test REFPROP function calls, problem goes away.
NonCO2CycleIndependentVariableLabels+=('Fuel Cell Fuel Utilization [%]',)
NonCO2CycleIndependentVariableMappings+=("CycleInputParameters['CombinedFuelCellAndCombustor']['FuelCellFuelUtilization']",)
...
if isfile('./scratch/OptimizedCombinedCycleResult.p'):
	...
	AllEnginesInitialGuessIndependentVariableValues=ReplicateCombinedCycleInputs(NonCO2CycleInitialGuessIndependentVariableValues,InitialGuessIndependentVariableValues,NumberOfEngines)
else:
	#use whatever is already in memory in the interpreter from a previous run.
	pass

AllEnginesIndependentVariableValueLimits=ReplicateCombinedCycleInputs(NonCO2CycleIndependentVariableValueLimits,IndependentVariableValueLimits,NumberOfEngines)
AllEnginesFixedIndependentVariableValues=ReplicateCombinedCycleInputs(NonCO2CycleFixedIndependentVariableValues,CO2CycleFixedIndependentVariableValues,NumberOfEngines)
...
ValueCount=(NumberOfEngines,len(NonCO2CycleInitialGuessIndependentVariableValues),len(NonCO2CycleFixedIndependentVariableValues),0)
...
WriteAdditionalObjects((NonCO2CycleFixedIndependentVariableValues,CO2CycleFixedIndependentVariableValues),NonCO2CycleIndependentVariableLabels,ValueCount)

#run the optimization
CombinedCycleEfficiency,AllEnginesOptimizedIndependentVariableValues=OptimizeCombinedCycle(AllEnginesInitialGuessIndependentVariableValues,AllEnginesFixedIndependentVariableValues,ValueCount,LineColors,CombinedOptimization,AllEnginesIndependentVariableValueLimits)	#save AllEnginesInitialGuessIndependentVariableValues so it can be passed back later to get a better initial condition

WriteResultObject((CombinedCycleEfficiency,)+AllEnginesOptimizedIndependentVariableValues)
